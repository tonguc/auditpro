...
const IMPACT_MAP = [
  { keys:['lcp','loading speed','page speed','core web vitals','cls','fid','inp','slow'],
    impact:'Slow loading directly reduces search rankings and increases user drop-off. Google treats Core Web Vitals as a ranking signal.',
    fix:'Optimise images, defer unused JavaScript, and improve server response time. Target LCP under 2.5s.' },
  { keys:['https','ssl','mixed content','security','http'],
    impact:'Security issues reduce user trust and negatively impact SEO. Google marks non-HTTPS pages as "Not Secure".',
    fix:'Ensure all pages redirect to HTTPS and remove any mixed content references.' },
  { keys:['meta description','description missing'],
    impact:'Missing or duplicate meta descriptions reduce click-through rates from search results, lowering organic traffic.',
    fix:'Write unique, compelling meta descriptions (140–160 characters) for all key pages.' },
  { keys:['title tag','page title','title missing','title duplicate'],
    impact:'Weak or missing title tags reduce search visibility and click-through rates from search results.',
    fix:'Write unique, keyword-rich title tags (50–60 characters) for every important page.' },
  { keys:['schema','structured data','json-ld','markup'],
    impact:'Missing structured data reduces eligibility for rich search results, lowering visibility and CTR.',
    fix:'Implement appropriate Schema.org markup (Organization, Product, Article, FAQ) for key content types.' },
  { keys:['robots','ai crawler','gptbot','claudebot','perplexitybot','crawl blocked'],
    impact:'Blocking AI crawlers prevents the site from being indexed by AI-driven search engines — a fast-growing traffic source.',
    fix:'Review robots.txt and allow GPTBot, ClaudeBot, and PerplexityBot unless intentionally blocking them.' },
  { keys:['mobile','responsive','viewport','touch'],
    impact:'Poor mobile experience drives away the majority of web users and impacts Google\'s mobile-first ranking.',
    fix:'Ensure all pages are fully responsive with correct viewport settings and touch-friendly interactive elements.' },
  { keys:['h1','heading structure','headings','h2','h3'],
    impact:'Poor heading structure confuses search engines and makes content harder to scan for users.',
    fix:'Use a clear heading hierarchy: one H1 per page, followed by descriptive H2 and H3 sub-sections.' },
  { keys:['canonical','duplicate content','duplicate url'],
    impact:'Duplicate content dilutes ranking signals and may cause search engines to index the wrong page version.',
    fix:'Add canonical tags to all pages and resolve duplicate content issues at the URL level.' },
  { keys:['alt text','image alt','alt attribute'],
    impact:'Missing alt text harms accessibility and prevents images from appearing in image search results.',
    fix:'Add descriptive, keyword-relevant alt text to all meaningful images on the site.' },
  { keys:['author','byline','bio','authorship'],
    impact:'Missing author attribution weakens E-E-A-T signals, reducing trust with both users and AI search engines.',
    fix:'Add author names, credentials, and biographical information to all content and article pages.' },
  { keys:['llms.txt','ai-readable','ai content structure'],
    impact:'Without AI-readable content structure, AI tools cannot easily parse, cite, or recommend your content.',
    fix:'Add an /llms.txt file and structure key content with clear headings, direct answers, and summary introductions.' },
  { keys:['brand','knowledge graph','entity','wikidata'],
    impact:'Weak entity signals make it harder for AI search engines to identify, trust, and recommend your brand.',
    fix:'Create a Google Business Profile, add Organization schema with sameAs links, and build brand mentions on authoritative sites.' },
  { keys:['cta','call to action','primary button'],
    impact:'Unclear or missing calls-to-action directly reduce conversions and revenue from existing traffic.',
    fix:'Ensure each key page has one clear primary CTA, with action-oriented language and strong visual contrast.' },
  { keys:['trust','review','testimonial','social proof','credibility'],
    impact:'Lack of trust signals increases visitor doubt and reduces conversion rates, especially for first-time visitors.',
    fix:'Add customer testimonials, reviews, case studies, or trust badges to key landing and conversion pages.' },
  { keys:['404','broken link','error page'],
    impact:'Broken links damage user experience and waste crawl budget, reducing overall SEO performance.',
    fix:'Fix or redirect all broken internal links. Ensure the 404 page is helpful and links back to key content.' },
  { keys:['sitemap','xml sitemap'],
    impact:'A missing or outdated sitemap makes it harder for search engines to discover and index all pages.',
    fix:'Generate an updated XML sitemap and submit it via Google Search Console.' },
  { keys:['internal link','internal linking'],
    impact:'Poor internal linking limits how link authority flows through the site and reduces discoverability of key pages.',
    fix:'Add contextual internal links from high-traffic pages to key conversion or category pages.' },
];
...
function getImpactAndFix(itemText, howTo) {
  ...
  for (const entry of IMPACT_MAP) {
    if (entry.keys.some(k => combined.includes(k))) {
      return { impact: entry.impact, fix: entry.fix };
    }
  }
  return {
    impact: 'This issue is reducing site performance and should be resolved to improve overall quality and user experience.',
    fix: howTo || 'Review and address this item based on the audit guidance provided.',
  };
}
...
function generateReportCopy(score, results) {
  ...
  const executiveSummary =
    `This website demonstrates ${PERF_LEVEL(overall)} performance with an overall score of ${overall}/100 (Grade ${score.grade}). ` +
    `The biggest opportunity lies in improving ${weakestMeaning}, which is the weakest area in this audit. ` +
    `There are ${urgencyText}. ` +
    `Addressing the top priority issues first will deliver the fastest, most measurable improvements to both search performance and user experience.`;

  const performanceInterpretation = `This score indicates ${PERF_MEANING(overall)}.`;

  const confLevel = compRate < 20
    ? 'directional rather than definitive — review more items for reliable conclusions'
    : compRate < 60
    ? 'moderately reliable — results give a useful picture of site health, though some areas may need further review'
    : 'reliable — a strong majority of checkpoints have been reviewed, making this a solid basis for action';
  const confidenceText =
    `This audit covers ${compRate}% of all checkpoints (${evalCount} of ${totalCount} items reviewed). Results should be treated as ${confLevel}.`;
  ...
  const priorityIssues = AUDIT_CATEGORIES.flatMap(cat =>
    cat.sections.flatMap(sec => sec.items
      ...
      .map(it => {
        const { impact, fix } = getImpactAndFix(it.item, it.howTo);
        return { ...it, catLabel: cat.label, catRgb: hexToRgb(cat.color), impact, fix };
      })
    )
  ).sort((a,b) => (PSCORE[b.priority]??0) - (PSCORE[a.priority]??0));

  return { executiveSummary, performanceInterpretation, confidenceText, sectionInsights, priorityIssues };
}
...
function downloadPDF(config, auditUrl, score, results) {
  ...
  if (enrichedIssues.length > 0) {
    ...
    enrichedIssues.slice(0, 6).forEach((item, i) => {
      ...
      doc.text(titleLine[0], M + 2, y + 11.5);
      // Why it matters
