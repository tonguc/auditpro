...
function downloadPDF(config, auditUrl, score, results) {
  ...
  if (copy) {
    ...
    const estImpactText = copy.weakestId ? ESTIMATED_IMPACT[copy.weakestId] : null;
    if (estImpactText) {
      const eiLines = doc.splitTextToSize(estImpactText, W - 2*M - 38);
      const eiH = Math.max(14, 8 + eiLines.length * 4);
